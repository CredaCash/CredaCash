...
BlockAux* Block::SetupAuxBuf(SmartBuf smartobj)
{
	...
	if (TRACE_SMARTBUF) BOOST_LOG_TRIVIAL(debug) << "Block::SetupAuxBuf level " << 	wire->level << " witness " << (unsigned)wire->witness << " prior oid " << buf2hex(&wire->prior_oid, sizeof(ccoid_t));
...
void Block::ChainToPriorBlock(SmartBuf priorobj)
{
	...
	if (TRACE_BLOCK) BOOST_LOG_TRIVIAL(trace) << "BlockAux::ChainToPriorBlock level " << wire->level << " nwitnesses " << auxp->blockchain_params.nwitnesses << " witness " << (unsigned)wire->witness << " skip " << auxp->skip << " prior witness " << (unsigned)prior_wire->witness << " prior oid " << buf2hex(&prior_auxp->oid, sizeof(ccoid_t));
...
bool Block::CheckBadSigOrder(int top_witness) const
{
	...
	if (TRACE_CHECKORDER) BOOST_LOG_TRIVIAL(trace) << "Block::CheckBadSigOrder top_witness " << top_witness << " starting at level " << wire->level << " nwitnesses " << nwitnesses << " maxmal " << auxp->blockchain_params.maxmal << " nconfsigs " << nconfsigs << " witness " << (unsigned)wire->witness << " skip " << auxp->skip << " oid " << buf2hex(&auxp->oid, sizeof(ccoid_t));
	...
	while (nsigs < nconfsigs)
	{
		...
		if (!prior_block)
		{
			if (wire->level)
			{
				BOOST_LOG_TRIVIAL(error) << "Block::CheckBadSigOrder no prior block at level " << wire->level << " skip " << auxp->skip << " nsigs " << nsigs << " skipsum " << skipsum;
	...
	{
		if (TRACE_CALCSKIPSCORE) BOOST_LOG_TRIVIAL(trace) << "Block::CheckBadSigOrder block violates dupe/ordering rule; level " << wire->level << " nwitnesses " << nwitnesses << " nsigs " << nsigs << " skipsum " << skipsum << " oid " << buf2hex(&AuxPtr()->oid, sizeof(ccoid_t));
...
uint64_t Block::CalcSkipScore(int top_witness, SmartBuf last_indelible_block, uint16_t genstamp, bool maltest)
{
	...
	if (maltest)
	{
		auto target_level = last_indelible_wire->level;
		...
		if (wire->level <= target_level)
		{
			if (TRACE_CALCSKIPSCORE) BOOST_LOG_TRIVIAL(trace) << "Block::CalcSkipScore top witness " << top_witness << " test mal " << maltest << " starting at level " << wire->level << " <= target level " << target_level << " returning score 0";

			return 0;
		}
	}

	if (TRACE_CALCSKIPSCORE) BOOST_LOG_TRIVIAL(trace) << "Block::CalcSkipScore top witness " << top_witness << " test mal " << maltest << " starting at level " << wire->level << " nwitnesses " << auxp->blockchain_params.nwitnesses << " maxmal " << auxp->blockchain_params.maxmal << " nconfsigs " << auxp->blockchain_params.nconfsigs << " witness " << (unsigned)wire->witness << " skip " << auxp->skip << " oid " << buf2hex(&auxp->oid, sizeof(ccoid_t)) << " last indelible level " << last_indelible_wire->level;
	...
	if (TRACE_CALCSKIPSCORE) BOOST_LOG_TRIVIAL(trace) << "Block::CalcSkipScore score " << hex << score << dec << " test mal " << maltest << " starting at level " << wire->level << " nwitnesses " << auxp->blockchain_params.nwitnesses << " maxmal " << auxp->blockchain_params.maxmal << " nconfsigs " << auxp->blockchain_params.nconfsigs << " witness " << (unsigned)wire->witness << " skip " << auxp->skip << " oid " << buf2hex(&auxp->oid, sizeof(ccoid_t)) << " last indelible level " << last_indelible_wire->level;
...
void Block::CalcSkipScoreRecursive(const BlockWireHeader* last_indelible_wire, uint16_t genstamp, bool maltest, uint64_t& score, unsigned& scorebits)
{
	...
	if (wire->level <= last_indelible_wire->level && !maltest)
	{
		score = 0;
		scorebits = 0;
	...
	if (maltest)
	{
		...
		if (wire->level <= target_level)
		{
			...
			if (TRACE_CALCSKIPSCORE) BOOST_LOG_TRIVIAL(trace) << "Block::CalcSkipScore test mal " << maltest << " reached level " << wire->level << " returning score " << hex << score << dec << " scorebits " << scorebits;
	...
	if (!prior_block)
	{
		...
		if (TRACE_CALCSKIPSCORE) BOOST_LOG_TRIVIAL(trace) << "Block::CalcSkipScore test mal " << maltest << " no prior block at level " << wire->level << " returning score " << hex << score << dec << " scorebits " << scorebits;
	...
	if (TRACE_CALCSKIPSCORE) BOOST_LOG_TRIVIAL(trace) << "Block::CalcSkipScore level " << wire->level << " added skip " << auxp->skip << " score " << hex << score << dec << " scorebits " << scorebits;
...
{
	auto rc = blake2b(&block_hash, sizeof(block_hash), NULL, 0, BodyPtr() + sizeof(block_signature_t), BodySize() - sizeof(block_signature_t));
	CCASSERTZ(rc);

	if (TRACE_CALCOID) BOOST_LOG_TRIVIAL(trace) << "Block::CalcHash TRACE_CALCOID for block level " << WireData()->level << " size " << ObjSize() << " prior oid " << buf2hex(&(WireData()->prior_oid), sizeof(WireData()->prior_oid)) << " computed block hash " << buf2hex(&block_hash, sizeof(block_hash));
}
...
void Block::CalcOid(const block_hash_t& block_hash, ccoid_t& oid)
{
	...
	int rc = blake2s_init(&ctx, sizeof(ccoid_t), NULL, 0);
	CCASSERTZ(rc);
	...
	*(uint32_t*)&oid = seq.fetch_add(1);
	...
	if (TRACE_CALCOID) BOOST_LOG_TRIVIAL(trace) << "Block::CalcOid TRACE_CALCOID for block level " << WireData()->level << " size " << ObjSize() << " prior oid " << buf2hex(&(WireData()->prior_oid), sizeof(WireData()->prior_oid)) << " from block hash " << buf2hex(&block_hash, sizeof(block_hash)) << " computed oid " << buf2hex(&oid, sizeof(oid));
}
...
bool Block::SignOrVerify(bool verify)
{
	...
	if (verify)
	{
		...
		if (TRACE_SIGNING)
		{
			BOOST_LOG_TRIVIAL(debug) << "Block::SignOrVerify verify level " << wire->level << " witness " << (unsigned)wire->witness;
	...
	else
	{
		...
		if (TRACE_SIGNING)
		{
			...
			BOOST_LOG_TRIVIAL(debug) << "Block::SignOrVerify sign level " << wire->level << " witness " << (unsigned)wire->witness;
