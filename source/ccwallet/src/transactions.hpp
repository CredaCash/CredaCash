...
private:
	...
	static int WaitNewBillet(const snarkfront::bigint_t& total_required, const uint64_t billet_count, const uint64_t t0, unique_lock<mutex>& lock);
	...
	static int TryCreateTxPay(DbConn *dbconn, TxQuery& txquery, TxParams& txparams, uint64_t dest_chain, Secret &destination, const snarkfront::bigint_t& amount, const string& comment, const string& comment_to, const bool subfee, const uint64_t t0, snarkfront::bigint_t& round_up, deque<Transaction>& tx_list, unique_lock<mutex>& lock);
...
public:
	...
	static int UpdateStatus(DbConn *dbconn, uint64_t tx_id, uint64_t bill_id, uint64_t commitnum);
};
